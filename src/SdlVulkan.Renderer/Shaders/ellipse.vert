...
// vertex stage doesn't read strokeWidth but it's still part of the block so the
...
//
// aLocalPos is the vertex's coordinate under the inverse of the affine map, and
// the quad is padded past the unit square by the stroke's outer half plus one
// pixel of edge (VkRenderer.EllipseQuad), so it runs a little beyond +-1; the
// fragment side reads only its length.
